...
function DashboardContent() {
    ...
    if (user.role === UserRole.ADMIN) {
      return [
        { 
          ...
          value: `$${dashboardStats.monthlyRevenue.toLocaleString('es-AR')}`, 
          icon: CreditCard, 
  ...
  const getAlerts = () => {
    if (user.role === UserRole.ADMIN) {
      return [
        { type: "warning", message: "5 pagos vencidos requieren atención", action: "Ver pagos", href: "/payments" },
        { type: "info", message: "3 nuevas solicitudes de inscripción", action: "Revisar", href: "/clients" },
      ]
    } else if (user.role === UserRole.TRAINER) {
      return [
        ...
        {alerts.length > 0 && (
          <div className="space-y-3">
            {alerts.map((alert, index) => (
              <Card
                key={index}
                className={`border-l-4 shadow-professional transition-all duration-200 hover:shadow-professional-lg ${
                  alert.type === "warning"
