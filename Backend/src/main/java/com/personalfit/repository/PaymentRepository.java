...
public interface PaymentRepository extends JpaRepository<Payment, Long> {
    ...
    @Query("SELECT p FROM Payment p " +
           "JOIN FETCH p.paymentUsers pu " +
           "WHERE pu.user = :user AND p.status = :status " +
           "ORDER BY p.createdAt DESC " +
           "LIMIT 1")
    Optional<Payment> findTopByUserAndStatusOrderByCreatedAtDesc(@Param("user") User user, @Param("status") PaymentStatus status);
    ...
    @Query("SELECT COUNT(p) > 0 FROM Payment p " +
           "JOIN p.paymentUsers pu " +
           "WHERE p.id = :paymentId AND pu.user.id = :userId")
