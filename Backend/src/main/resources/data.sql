...
-- -- Agregar nuevos campos a la tabla de notificaciones
...
-- ALTER TABLE notification ADD COLUMN IF NOT EXISTS info_type VARCHAR(20) DEFAULT 'info';
-- ALTER TABLE notification ADD COLUMN IF NOT EXISTS notification_category VARCHAR(50);

-- -- Actualizar notificaciones existentes para que tengan valores por defecto
-- UPDATE notification SET read = FALSE WHERE read IS NULL;
-- UPDATE notification SET archived = FALSE WHERE archived IS NULL;
-- UPDATE notification SET info_type = 'info' WHERE info_type IS NULL; 